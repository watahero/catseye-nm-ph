...
export class Repo {
  ...
  async loadSql() {
    ...
    const { text, branch } = await tryBranches("sql/mob_spawn_points.sql", ["mods", "base", "main", "master"]);
    ...
    try {
      const r = await tryBranches("sql/zone_settings.sql", ["mods", "base", "main", "master"]);
